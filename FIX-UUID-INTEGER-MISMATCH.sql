-- ============================================================================
-- ZAI FLOW 2.0 - FIX UUID-INTEGER USER ID MISMATCH
-- This script resolves the core issue where RPC functions receive Supabase Auth
-- UUIDs from the frontend but the database uses INTEGER user_ids
-- ============================================================================
...
-- Now accepts UUID from frontend and converts to INTEGER for database lookup
-- ============================================================================
...
BEGIN
  ...
  v_user_id := public.get_user_id_from_auth(p_auth_id);
...
-- ============================================================================
-- 5. OTHER FUNCTIONS THAT NEED UUID-TO-INTEGER CONVERSION
-- ============================================================================
...
-- ============================================================================
-- 5B. ASSIGN ROLE TO USER
-- Updated to convert UUID to INTEGER for user_roles table
-- ============================================================================
...
BEGIN
  ...
  v_user_id := public.get_user_id_from_auth(p_auth_id);
  v_assigned_by_id := public.get_user_id_from_auth(p_assigned_by_auth_id);
  ...
  VALUES (v_user_id, p_business_id, p_role_id, p_assigned_by_auth_id)
...
-- ============================================================================
-- 6. NOTIFICATIONS - ENSURE UUID HANDLING
-- ============================================================================

-- CREATE NOTIFICATION - Accept UUID and keep as UUID for notifications table
DROP FUNCTION IF EXISTS public.create_notification(UUID, INTEGER, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ) CASCADE;
...
BEGIN
  ...
  VALUES (
    p_auth_id, p_business_id, p_title, p_message, p_type, p_action_url, p_expires_at
  )
