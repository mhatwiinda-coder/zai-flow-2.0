...
const filename = 'ZAI-FLOW-BROCHURE-PREMIUM.pdf';
...
function drawGradientRect(x, y, w, h, color1, color2) {
  doc.fillColor(color1).rect(x, y, w, h).fill();
}
...
// Stats
doc.fontSize(11).font('Helvetica-Bold').fillColor(gold).text('✓ 2-Week Implementation  ✓ 24/7 Local Support  ✓ 30-Day Money-Back', 50, 450);
...
const features = [
  ...
  },
  {
    ...
    functions: [
      ...
      'Stock transfers & reconciliation',
...
features.forEach((feat, idx) => {
  ...
  // Icon & Title
  doc.fontSize(28).text(feat.icon, xPos + 15, featureY + 12);
  doc.fontSize(12).font('Helvetica-Bold').fillColor(turquoise).text(feat.title, xPos + 55, featureY + 20);

  // Functions list
  let funcY = featureY + 50;
...
keyFeatures.forEach(kf => {
  ...
  [kf.zaiflow, kf.zamtech, kf.africaflow, kf.smartbiz].forEach(has => {
    if (has) {
      doc.fontSize(12).font('Helvetica-Bold').fillColor(turquoise).text('✓', checkX, featureTableY);
    } else {
      doc.fontSize(12).font('Helvetica-Bold').fillColor('#666').text('✗', checkX, featureTableY);
    }
    checkX += 75;
  });
...
const plans = [
  ...
  },
  {
    ...
    price: 45000,
    users: 999,
    features: ['Everything', '24/7 support', 'Dedicated mgr', 'On-site training', 'Custom dev', 'White-label'],
    color: gold
  }
];
...
const contactCards = [
  ...
  { emoji: '🌐', label: 'Website', value: contact.website }
];
...
contactCards.forEach(card => {
  ...
  // Content
  doc.fontSize(16).text(card.emoji, 65, contactY + 15);
  doc.fontSize(11).font('Helvetica-Bold').fillColor(turquoise).text(card.label, 105, contactY + 12);
  doc.fontSize(11).font('Helvetica').fillColor(white).text(card.value, 105, contactY + 32);
...
console.log(`✅ Premium Brochure with real competitors generated: ${filename}`);
