-- ============================================================================
-- ZAI FLOW 2.0 - Supabase RPC Functions
...
CREATE OR REPLACE FUNCTION get_drawer_status(p_user_id INTEGER)
RETURNS TABLE (
  isOpen BOOLEAN,
...
CREATE OR REPLACE FUNCTION open_cash_drawer(p_user_id INTEGER, p_opening_balance NUMERIC)
RETURNS TABLE (
  opened BOOLEAN,
  drawer_id INTEGER
) AS $$
DECLARE
  v_drawer_id INTEGER;
BEGIN
  INSERT INTO public.cash_drawer (user_id, opening_balance, status)
  VALUES (p_user_id, p_opening_balance, 'OPEN')
...
BEGIN
  ...
  FROM public.sales
  WHERE created_at >= (SELECT created_at FROM public.cash_drawer WHERE id = p_drawer_id)
...
-- Test get_drawer_status (assumes user_id = 1 exists)
