...
const filename = 'ZAI-FLOW-PRICING-PREMIUM.pdf';
...
function addSubtitle(text) {
  doc.fontSize(16).font('Helvetica-Bold').fillColor(white).text(text);
  doc.moveDown(0.3);
}
...
addBullet('Free setup & training included');
...
const services = [
  { service: 'System Setup & Configuration', price: 'ZMW 50,000', detail: 'Initial setup, security, customization' },
...
const smallItems = [
  { item: 'Monthly Subscription (Starter)', detail: 'ZMW 5,000/month', value: 'ZMW 60,000' },
  { item: 'Setup & Implementation (one-time)', detail: 'System setup, data migration, training', value: 'ZMW 155,000' }
];
...
const medItems = [
  { item: 'Monthly Subscription (Professional)', detail: 'ZMW 15,000/month', value: 'ZMW 180,000' },
  { item: 'Setup & Implementation (one-time)', detail: 'System setup, data migration, training', value: 'ZMW 200,000' },
  { item: 'Integration Service (optional)', detail: 'Connect to existing systems', value: 'ZMW 60,000' }
];
...
addTitle('Payment Options & Flexibility');
...
const paymentPlans = [
  {
    ...
    features: ['Most flexible', 'No commitment', 'Cancel with 30 days notice']
  },
  {
    name: 'Quarterly',
    desc: 'Save 5% with quarterly billing',
    features: ['5% discount', 'Better budgeting', 'Quarterly commitment']
  },
  {
    name: 'Annual',
    desc: 'Save 10% with annual payment',
    features: ['10% discount', 'Maximum savings', 'Annual commitment']
  }
];

let paymentY = doc.y + 15;
paymentPlans.forEach(plan => {
  doc.fillColor(purple).fillOpacity(0.15).rect(50, paymentY, 495, 70).fill();
  doc.strokeColor(turquoise).lineWidth(1).rect(50, paymentY, 495, 70).stroke();
  ...
  doc.fontSize(9).font('Helvetica').fillColor(lightGray).text(plan.features.join(' • '), 65, paymentY + 48);
...
const methods = ['Bank Transfer (All major banks)', 'Mobile Money (MTN, Airtel)', 'Debit/Credit Card', 'Cheque (large orders)'];
...
contactDetails.forEach(c => {
  doc.fontSize(11).font('Helvetica').fillColor(lightGray).text(c.icon + '  ' + c.label + ': ' + c.value);
  doc.moveDown(0.4);
});
...
console.log(`✅ Premium Pricing Guide generated: ${filename}`);
